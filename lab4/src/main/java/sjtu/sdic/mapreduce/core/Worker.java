...
public class Worker implements WorkerRpcService {
  ...
  /**
   * Shutdown is called by the master when all work has been completed. We should respond with the
   * number of tasks we have processed.
   */
  @Override
  public int shutdown() {
  ...
  public void doTask(DoTaskArgs arg) {
    System.out.println(
        String.format(
            "%s: given %s task #%d on file %s (nios: %d)",
    ...
    switch (arg.phase) {
      ...
      case REDUCE_PHASE:
        Reducer.doReduce(
            arg.jobName,
            arg.taskNum,
            Utils.mergeName(arg.jobName, arg.taskNum),
            arg.numOtherPhase,
            reduceF);
        break;
    }
  ...
  /**
   * RunWorker sets up a connection with the master, registers its address, and waits for tasks to
   * be scheduled.
   *
  ...
  /**
   * generate a valid RPC service port according to the worker address (name)
   *
   * <p>Note: there may be port CONFLICTS with little probability if u encounter with this, try to
   * rename the worker
   *
  ...
  private void run(String master) {
    ...
    providerConfig =
        new ProviderConfig<WorkerRpcService>()
