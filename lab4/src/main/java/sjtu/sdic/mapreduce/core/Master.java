 ...
 *
 * <p>Created by Cachhe on 2019/4/19.
 */
public class Master implements MasterRpcService {
  ...
  /**
   * Sequential runs map and reduce tasks sequentially, waiting for each task to complete before
   * running the next.
   *
   ...
   */
  public static Master sequential(
      String jobName, String[] files, int nReduce, MapFunc mapF, ReduceFunc reduceF) {
  ...
  /**
   * register is an RPC method that is called by workers after they have started up to report that
   * they are ready to receive tasks.
   *
  ...
  /**
   * helper function that sends information about all existing and newly registered workers to
   ...
   *
   * <p>the params are similar to {@link Master#sequential(String, String[], int, MapFunc,
   * ReduceFunc)} except that {@code master} is the master's address
   *
   * @param jobName
   * @param files
   * @param nReduce
   ...
   *
   * <p>First, it divides up the input file among the given number of mappers, and schedules each
   * task on workers as they become available. Each map task bins its output in a number of bins
   * equal to the given number of reduce tasks. Once all the mappers have finished, workers are
   * assigned reduce tasks.
   *
   * <p>When all tasks have been completed, the reducer outputs are merged, statistics are
   * collected, and the master is shut down.
   *
   * <p>Note that this implementation assumes a shared file system.
   *
   * @param jobName
   * @param files
   * @param nReduce
  ...
  public void run(
    ...
    System.out.println(
        String.format("%s: Starting Map/Reduce task %s", this.address, this.jobName));
  ...
  /**
   * mWait blocks until the currently scheduled work has completed. This happens when all tasks have
   * scheduled and completed, the final output have been computed, and all workers have been shut
   * down.
   */
  public void mWait() {
    try {
  ...
  private void startRPCServer() {
    ...
    if (rpc == null) {
      ...
      rpc =
          new ProviderConfig<MasterRpcService>()
  ...
  /**
   * stopRPCServer stops the master RPC server. This must be done through an RPC to avoid race
   * conditions between the RPC server thread and the current thread.
   */
      ...
      try {
        ...
        JSONObject json =
            JSONObject.parseObject(
                new String(Files.readAllBytes(new File(p).toPath()), StandardCharsets.UTF_8),
